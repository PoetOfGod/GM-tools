...
class ImageWindow(ttk.Frame):
    """ Main window class """
    def __init__(self, mainframe, path, winSize, title):
        """ Initialize the main Frame """
        ttk.Frame.__init__(self, master=mainframe)
        self.master.title(title)
        self.master.geometry(winSize)  # size of the main window
...
if __name__ == '__main__':
    filename = './data/monster/img/Archmage.webp'  # place path to your image here
    original_image = Image.open(filename)
    geometry = f"{original_image.width+17}x{original_image.height+17}"
    app = ImageWindow(tk.Tk(), path=filename, winSize=geometry, title="Archmage")
    app.mainloop()